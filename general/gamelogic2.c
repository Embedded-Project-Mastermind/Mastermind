...
void insert_color(char color){
    ...
    print_sequence((char*)tentative.seq_user, tentative.count);  //to print sequence after adding an element
}
...
void eliminate_color(void){  //eliminates color in position tentative.count from seq_user array
     ...
     print_sequence((char*)tentative.seq_user, tentative.count);  //to print sequence after eliminating an element
}
